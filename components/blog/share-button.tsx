...
const SHARE_PROVIDERS = [
  {
    ...
    icon: (
      <svg className="size-5 text-white dark:text-black" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    ...
    getUrl: (url: string, title: string) => `https://wa.me/?text=${title}%20${url}`,
  },
  {
    label: "Telegram",
    color: "bg-sky-500",
    icon: <HugeiconsIcon icon={SentIcon} className="size-5 text-white" strokeWidth={2} />,
    getUrl: (url: string, title: string) => `https://t.me/share/url?url=${url}&text=${title}`,
  },
  {
    label: "Email",
    color: "bg-orange-500",
    icon: <HugeiconsIcon icon={Mail01Icon} className="size-5 text-white" strokeWidth={2} />,
    getUrl: (url: string, title: string) => `mailto:?subject=${title}&body=Check out this post: ${url}`,
    target: "_self"
  },
];
          ...
          <div className="grid grid-cols-3 gap-0.5 sm:grid-cols-6 sm:gap-1">
            {SHARE_PROVIDERS.map((option) => (
              <ShareOption
                ...
                onClick={() => {
                  window.open(
                    option.getUrl(encodedUrl, encodedTitle),
                    option.target || "_blank",
                    option.target === "_self" ? undefined : "noopener,noreferrer"
                  );
                }}
              />
            ))}
